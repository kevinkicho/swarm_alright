...
function archiveRoleDump(opts: {
  ...
  try {
    ...
    const base = `${opts.role}-c${opts.cycle}-${safeSeg(opts.tag)}-${sid}-${stamp}.md`
    const dest = path.join(dir, base)

    const header = [
      `<!-- archive role=${opts.role} tag=${opts.tag} cycle=${opts.cycle} session=${opts.meta?.sessionID ?? "?"} -->`,
      `<!-- messages=${opts.meta?.messageCount ?? "?"} tools=${opts.meta?.toolCalls ?? "?"} errors=${opts.meta?.toolErrors ?? "?"} -->`,
      "",
    ].join("\n")
    fs.writeFileSync(dest, header + body)
    const cycleLatest = path.join(dir, `${opts.role}-c${opts.cycle}-latest.md`)
...
export function archiveWorkerSessionDump(opts: {
  ...
  return archiveRoleDump({ ...opts, role: "worker" })
}
...
export function archiveSystemSessionDump(opts: {
  ...
  return archiveRoleDump({ ...opts, role: "system", minChars: 40 })
}
