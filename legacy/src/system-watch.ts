 ...
 *
 * Token strategy: digests are injected every 3 minutes during worker turns (not
 * every 25s), so the system gets real-time awareness without excessive context
 * growth. One final digest at end-of-turn ensures the system has the full
 * picture. Alerts still trigger active watch turns immediately.
 */
...
export class SystemWatch {
  ...
  constructor(opts: SystemWatchOpts) {
    this.opts = opts
    this.injectIntervalMs = opts.injectIntervalMs ?? 180_000
   ...
   * Injects digests every 3 minutes for real-time awareness; on alerts may run
   * a short lead watch turn. Returns stopWorker if lead said HOST: STOP.
   */
  async runWhile(
    deps: TurnDeps,
    ...
    await this.injectNow(
      [
        ...
        `Host fans OpenCode events into this session as digests (noReply) every ~3m.`,
    ...
    while (!isWorkerDone() && !this.opts.isStopping() && !this.stopped) {
      ...
      // Periodic digest flush — every 3 minutes, not every 25s.
      ...
      if (
        ...
        if (!this.pending.some((l) => /alert:/i.test(l))) {
          this.pending.push(`[${new Date().toISOString().slice(11, 19)}] alert: (see BUS.md for details)`)
        }
  ...
  private async activeWatchTurn(
    ...
    const prompt = [
      ...
      `- HOST: DONE — only when mission goals are truly complete (ends the entire run).`,
      `Use STOP to unstick a hung sub; use DONE to close the mission.`,
    ...
    try {
      const identity = [
        ...
        `Prefer short intervention. Re-do the full materials review only when needed.`,
