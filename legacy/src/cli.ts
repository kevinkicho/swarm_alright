...
Usage:
  ...
  swarm panel [run-id]           Interactive control panel — live state + editable guards
...
async function cmdLogs(args: Args): Promise<void> {
  ...
  const pump = () => {
    ...
    } catch (err) { trace("cli.cmdLogs.pump", err) }
  }
  pump()
...
async function cmdPanel(args: Args): Promise<void> {
  const { panel } = await import("./panel.ts")
  await panel(args.positional[0])
}
