...
import { busJsonlPath, busMdPath } from "./run-paths.ts";
import { trace } from "./trace.ts";
export { busJsonlPath, busMdPath }
...
export function publishBusEvent(runDir: string, ev: Omit<BusEvent, "ts"> & { ts?: string }): void {
  ...
  } catch (err) { trace("eventBus.publishBusEvent", err) }
}
...
export function writeBusSnapshot(
  ...
  } catch (err) { trace("eventBus.writeBusSnapshot", err) }
}
...
export function loadBusRingFromDisk(runDir: string, max = 100): number {
  try {
    ...
    for (const line of rows) {
      ...
      } catch (err) { trace("eventBus.loadBusRingFromDisk", err) }
    }
    return ring.length
  } catch {
    return 0
