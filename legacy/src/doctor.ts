...
export async function printStatus(runId?: string): Promise<void> {
  ...
  for (const r of runs.slice(0, runId ? 1 : 12)) {
    ...
    } catch (err) { trace("doctor.printStatus.git", err) }
    ...
    } catch (err) { trace("doctor.printStatus.scorecard", err) }
...
export async function printDoctor(projectArg?: string): Promise<void> {
  ...
  } catch (err) { trace("doctor.printDoctor.dirtyPaths", err) }
  ...
  if (sample) {
    ...
    } catch (err) { trace("doctor.printDoctor.scorecard", err) }
  }
