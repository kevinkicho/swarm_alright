...
import { question, fetchModels } from "./shared.ts"
...
async function newRunFlow(): Promise<void> {
  ...
  } catch (err) { trace("wizard.newRunFlow.loadApiKey", err) }
...
async function modelsFlow(): Promise<void> {
  ...
  try {
    key = loadApiKey()
  } catch (err) { trace("wizard.modelsFlow.loadApiKey", err) }
      ...
      [
        ...
        ...(active.length
          ? [
              ...
              { label: "control panel", hint: "live state + editable guards & thresholds", value: "panel" },
